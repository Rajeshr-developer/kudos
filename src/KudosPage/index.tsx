...
class KudosPage extends Component<Props> {
    ...
    render() {
        ...
        console.log('this.props render = ', this.props.kudosData);
...
const UserInfoBlock = ({ data, uName, disp }: { data: jsoninfo, uName: any, disp: any }) => {
    ...
    return (
        <div style={{ 'height': '17vh', 'position': 'relative' }}>
            ...
            {
                data.name == uName && <ImageStyle key={data.id} style={{ 'left': '15%', 'cursor': 'pointer' }} src={delicon} onClick={(e: React.MouseEvent<HTMLElement>) => { disp(asyncReducer(undefined,'DELETE','REMOVE_JSON_DATA','Kudos/'+data.id)) }}></ImageStyle>
            }
            <ImageStyle style={{ 'left': '9%', 'cursor': 'pointer' }} src={handicon}></ImageStyle>
        </div>
    )
}
